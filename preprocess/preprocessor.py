...
from tools import feature_scale
from tools import reduce_impute_encode    
...
def create_data_set(filename, n_features = -1, columns = np.r_[:107, -1], rows = np.r_[:8378], test_size = 0.2, standarize = True):
    ...
    train_path = pathlib.Path('datasets/'+filename+"_train.csv")
    test_path = pathlib.Path('datasets/'+filename+"_test.csv")
